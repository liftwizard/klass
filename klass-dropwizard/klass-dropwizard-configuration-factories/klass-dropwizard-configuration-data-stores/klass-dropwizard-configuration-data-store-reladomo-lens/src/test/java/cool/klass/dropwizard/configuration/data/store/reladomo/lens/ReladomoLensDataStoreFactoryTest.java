...
class ReladomoLensDataStoreFactoryTest {
	...
	void isDiscoverable() {
		...
		assertThat(discoveredSubtypes).contains(ReladomoLensDataStoreFactory.class);
	}

	@Test
	void reladomoLensDataStore() throws Exception {
		DataStoreFactory dataStoreFactory = this.factory.build(
			new ResourceConfigurationSourceProvider(),
			"config-test.json5"
		);
		assertThat(dataStoreFactory).isInstanceOf(ReladomoLensDataStoreFactory.class);
	}
