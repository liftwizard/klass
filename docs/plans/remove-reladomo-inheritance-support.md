...
### Inheritance Strategy: Joined Tables with Discriminator

Each class in the hierarchy has its own table, with a **discriminator column** in the root table:

```
Person (id, name, classType)     -- base table with discriminator
...
| Property | Value |
|----------|-------|
| Column Name | `classType` (or `dtype`, `discriminator`) |
| Location | Root table of each hierarchy only |
| Type | `String` / `VARCHAR` |
| Value | Fully qualified class name (e.g., `"Manager"` or `"com.example.Manager"`) |
| Nullable | No - every row must have a type |
| Generated By | Klass, not Reladomo |

**Benefits of Discriminator Column:**

1. **Single query to determine type** - No need to probe subclass tables
2. **Efficient polymorphic queries** - Filter by discriminator to find specific subtypes
3. **Simpler deserialization** - Know the target type before constructing objects
4. **Standard JPA/Hibernate pattern** - Well-understood approach

**Example Queries with Discriminator:**

```sql
-- Find all Managers (know exactly which tables to join)
SELECT p.*, e.*, m.*
FROM Person p
JOIN Employee e ON p.id = e.id
JOIN Manager m ON e.id = m.id
WHERE p.classType = 'Manager'

-- Find all Employees (including Managers)
SELECT p.*, e.*, m.*
FROM Person p
JOIN Employee e ON p.id = e.id
LEFT JOIN Manager m ON e.id = m.id
WHERE p.classType IN ('Employee', 'Manager')

-- Polymorphic query on Person (all subtypes)
SELECT p.*, e.*, m.*
FROM Person p
...
This is the same table structure as today, plus the discriminator. Reladomo won't know about the inheritance relationship.

## Affected Layers

### 1. Schema Generation (Medium Impact)

**Current**: `TableGenerator.java` excludes superclass properties from subclass tables.

**Change**:
- Add discriminator column (`classType`) to root tables of inheritance hierarchies
- The discriminator column should be non-nullable with a reasonable max length
- Only the **root** of each hierarchy gets the discriminator (not intermediate abstract classes)

**Files**:
- `klass-generator-liquibase-schema/TableGenerator.java`
- Possibly `klass-generator-relational-schema/RelationalSchemaGenerator.java`
...
### Phase 1: Add Discriminator Column to Schema

1. Update `TableGenerator.java` to add `classType` column to root tables
2. Update Klass model to recognize/generate the discriminator property
3. Generate Liquibase migration for existing schemas (add column with default value)
4. Test schema generation with inheritance hierarchies

### Phase 2: Investigate Reladomo Behavior Without Inheritance
...
3. Verify:
   - Tables are still created correctly (now with discriminator)
   - Each class works independently in Reladomo
   - Understand what breaks when inheritance is removed

### Phase 3: Create Inheritance-Aware Query Layer
...
2. Ensure `__typeName` correctly reflects actual runtime type (use discriminator)

### Phase 6: Update Persistent Synchronizer

1. Implement insert across hierarchy (including discriminator):
   ```
   insert Person(id=1, name="Alice", classType="Manager")  -- discriminator set to leaf type
...
### Phase 7: Remove Reladomo Inheritance Generation
...
### Phase 8: Enable Bootstrap and Sample Data
...
**Option B**: Add discriminator column to base table (schema change) ← **SELECTED**

~~**Option C**: Maintain type registry/cache (complexity)~~

**Implementation Details:**
- Column name: `classType` (consistent with Klass terminology)
- Stored in root table of each inheritance hierarchy
- Value: Simple class name (e.g., `"Manager"`) - no package prefix needed since unique within hierarchy
- Set automatically on insert based on the concrete class being created
- Read on polymorphic queries to determine which subclass tables to join
...
1. ~~**Schema change acceptable?** Adding discriminator column would simplify type detection~~ ✅ **YES - using discriminator column**
...
5. **Discriminator column name?** Proposed: `classType` - confirm this works with existing conventions

## Appendix: Affected Modules Summary

**Must Change:**
- `klass-generator-liquibase-schema` - Add discriminator column to root tables
...
- `klass-reladomo-persistent-writer` - Multi-table insert/update/delete, set discriminator
