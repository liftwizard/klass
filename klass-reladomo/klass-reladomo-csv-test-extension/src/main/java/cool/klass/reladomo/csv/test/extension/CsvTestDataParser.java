...
public class CsvTestDataParser {
    ...
    @Nonnull
    private String extractClassNameFromFilename(@Nonnull String filename) {
        String baseFilename = filename;
        if (baseFilename.contains("/")) {
            baseFilename = baseFilename.substring(baseFilename.lastIndexOf('/') + 1);
        }
        if (!baseFilename.endsWith(".csv")) {
            throw new IllegalArgumentException("Filename must end with .csv: " + filename);
        }
    ...
    private void setValue(@Nonnull MithraDataObject dataObject, @Nonnull Attribute<?, ?> attribute, @Nonnull String value) {
        if (attribute instanceof TimestampAttribute timestampAttribute) {
